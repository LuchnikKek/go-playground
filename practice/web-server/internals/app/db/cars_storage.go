...
// Плоская структура, из которой будет собрана вложенная
type userCar struct {
...
func (storage *CarsStorage) GetCarsList(ctx context.Context, userIdFilter int64, brandFilter string, colourFilter string, licenseFilter string) []models.Car {
...
func (storage *CarsStorage) CreateCar(ctx context.Context, car models.Car) error {
	...
	if err != nil {
		log.Errorln(err)
		err = tx.Rollback(ctx) //если получили ошибку - откатываем транзакцию
		if err != nil {
			log.Errorln(err)
		}
		return err
